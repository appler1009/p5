...
extension NSImage {
  public func blurHash(numberOfComponents components: (Int, Int)) -> String? {
    ...
    guard let finalCgImage = context.makeImage(),
      let dataProvider = finalCgImage.dataProvider,
    ...
    let width = finalCgImage.width
    let height = finalCgImage.height
    ...
    for y in 0..<components.1 {
      for x in 0..<components.0 {
        ...
        let factor = multiplyBasisFunction(
          ...
          bytesPerPixel: finalCgImage.bitsPerPixel / 8, pixelOffset: 0
        ) {
...
private let encodeCharacters: [String] = {
  return "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz#$%*+,-.:;=?@[]^_{|}~".map {
    String($0)
  }
}()
